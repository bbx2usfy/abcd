...
def craftadv(modelConfig: Dict):
    ...
    set_targets = "targets_200"
    unknown = modelConfig["unknown"]

    if unknown=='False':
        src = "/home/GAKER/GAKer-main/Source samples for known classes test/"
    ...
    for target in targets:#        targets = [22, 30, 43, 51, 53, 67, 76, 84, 107, 111, 116, 139, 156, 163, 174, 191, 194, 199, 228, 241, 251, 288, 301, 310, 313, 323, 324, 354,393, 398, 399, 401, 405, 418, 419, 420, 422, 428, 429, 439, 441, 451, 455, 457, 465, 467, 478, 480, 481, 488, 489, 490, 493, 496, 498, 499, 500, 507, 508, 514, 515, 519, 523, 530, 532, 533, 539, 540, 550, 552, 553, 557, 565, 566, 575, 576, 579, 583, 588, 592, 593, 594, 599, 601, 604, 605, 606, 607, 608, 611, 614, 622, 627, 640, 644, 646, 647, 659, 660, 666, 668, 674, 678, 683, 684, 687, 688, 691, 694, 700, 704, 712, 714, 715, 722, 726, 729, 738, 739, 740, 741, 749, 751, 761, 766, 769, 772, 773, 783, 785, 789, 790, 793, 794, 796, 798, 800, 807, 815, 822, 825, 826, 831, 843, 844, 851, 853, 854, 855, 858, 860, 862, 863, 869, 876, 877, 879, 880, 884, 888, 891, 897, 898, 901, 903, 904, 908, 910, 912, 914, 916, 918, 919, 924, 925, 927, 931, 932, 933, 934, 937, 938, 943, 946, 950, 952, 954, 958, 959, 961, 963, 971, 974, 977, 979, 980, 984, 985, 995, 996]
        print('##########################')
        target_label = target
        numtotargetname = []
        ...
        ####################################################################################################
        if target_select=='10':
            target_img_dir = '/home/GAKER/GAKer-main/Target images dataset/'+target_name
        ...
        with torch.no_grad():
            for imgs,labels,file_name in source_set:
                ...
                # 动态生成保存路径并确保路径存在
                save_root = "./adv_samples"  # 根目录，用于保存所有对抗样本
                os.makedirs(save_root, exist_ok=True)

                # 为当前 target 类别生成专属保存路径
                target_folder = f"{save_root}/{target_name}"  # 每个类别单独的文件夹
                os.makedirs(target_folder, exist_ok=True)

                # 将对抗样本保存到类别文件夹中
                adv_img = (adv.detach().permute((0, 2, 3, 1)).cpu().numpy() * 255).astype(np.uint8)
                for j in range(adv_img.shape[0]):
                    save_path = os.path.join(target_folder, file_name[j])  # 确保文件名正确
                    Image.fromarray(adv_img[j]).save(save_path)
